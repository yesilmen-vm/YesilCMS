                                ...
                                <div class="blizzcms-item-container">
                                    <div class="blizzcms-item-header uk-text-truncate" uk-tooltip="<?= $items->name ?>" uk-toggle="target: #item-<?= $items->id ?>;animation: uk-animation-slide-top-small">
                                        <div class="item-store-icon">
                                            <img src="<?= base_url() ?>/application/modules/database/assets/images/icons/<?= $items->icon ?>.png" alt="icon">
                                        </div>
                                        <?php if ($items->type == 1): ?>
                                            <span class="uk-text-middle">
                        <a href="<?= base_url($lang) . '/item/' . $items->command ?>" class="item-store-href" data-item='item=<?= $items->command ?>' data-patch='10'><?= $items->name ?></a>
                      </span>
                                        <?php else: ?>
                                            <span class="uk-text-middle">
                                        ...
                                        <div class="uk-grid uk-grid-small uk-flex uk-flex-center" data-uk-grid>
                                            <div class="uk-width-auto">
                                                ...
                                                <?php elseif ($items->price_type == 3): ?>
                                                    <span class="blizzcms-item-price"><span uk-tooltip="title:<?= $this->lang->line('panel_dp'); ?>"><i class="dp-icon"></i></span><?= $items->dp ?> <span class="uk-badge">&amp;</span> <span uk-tooltip="title:<?= $this->lang->line('panel_vp'); ?>"><i
                                                                    class="vp-icon"></i></span><?= $items->vp ?></span>
                                                <?php endif; ?>
                                            </div>
                                            <div class="uk-width-auto">
