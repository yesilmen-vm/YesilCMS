      ...
      <div class="uk-container">
        <div class="uk-grid uk-grid-small uk-margin-small" data-uk-grid>
          ...
          <div class="uk-width-auto">
            <a href="<?= base_url('admin/menu'); ?>" class="uk-icon-button"><i class="fas fa-arrow-left"></i></a>
          </div>
        </div>
                ...
                <div class="uk-inline uk-width-1-2@s">
                  ...
                  <div class="uk-form-controls">
                    <select class="uk-select" id="menu_main">
                      ...
                      <option value="1" <?php if ($this->admin_model->getMenuSpecifyMain($idlink) == '1') {
                          echo 'selected';
                      } ?>><?= $this->lang->line('option_normal'); ?></option>
                      <option value="2" <?php if ($this->admin_model->getMenuSpecifyMain($idlink) == '2') {
                          echo 'selected';
                      } ?>><?= $this->lang->line('option_dropdown'); ?></option>
                    </select>
                  </div>
                </div>
              </div>
                ...
                <div class="uk-inline uk-width-1-2@s">
                  ...
                  <div class="uk-form-controls">
                    <select class="uk-select" id="menu_type">
                      <option value="0"><?= $this->lang->line('notification_select_type'); ?></option>
                      <option value="1" <?php if ($this->admin_model->getMenuSpecifyType($idlink) == '1') {
                          echo 'selected';
                      } ?>><?= $this->lang->line('option_internal_url'); ?></option>
                      <option value="2" <?php if ($this->admin_model->getMenuSpecifyType($idlink) == '2') {
                          echo 'selected';
                      } ?>><?= $this->lang->line('option_external_url'); ?></option>
                    </select>
                  </div>
                </div>
              </div>
    ...
    var csrfHash = "<?= $this->security->get_csrf_hash() ?>";
      ...
      function UpdateMenuForm(e) {
        ...
        $.ajax({
          ...
          method:"POST",
          data:{id, name, url, icon, main, child, type, [globalThis.csrfName]: globalThis.csrfHash},
          dataType:"text",
          beforeSend: function(){
            $.amaran({
              'theme': 'awesome info',
