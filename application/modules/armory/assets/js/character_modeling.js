...
const characterPart = () => {
    const ret = {
        ...
        "Skin Type": undefined,
        Ears: (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `ears`,
        "Fur Color": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `furColor`,
        Snout: `snout`,
        Blindfold: undefined,
        Tattoo: undefined,
        "Eye Color": undefined,
        ...
        Necklace: undefined,
        Earring: undefined,
        "Primary Color": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `primaryColor`,
        "Secondary Color Strength": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `secondaryColorStrength`,
        "Secondary Color": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `secondaryColor`,
        "Horn Color": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `hornColor`,
        Horns: (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `horns`,
        "Body Size": (window.YESILCMS_TO_RETAIL_DISPLAY_ID_API) ? undefined : `bodySize`
    }
    console.log(ret)
    return ret
}
 ...
 */
async function getDisplaySlot(item, slot, displayId, env=`classic`) {
    ...
    try {
        ...
        if(!window.YESILCMS_TO_RETAIL_DISPLAY_ID_API){
            throw Error(`Item not found and window.YESILCMS_TO_RETAIL_DISPLAY_ID_API not set`)
        }
        const resp = await fetch(`${window.YESILCMS_TO_RETAIL_DISPLAY_ID_API}/${item}`)
            .then((response) => response.json())
 ...
 */
async function findItemsInEquipments(equipments, env=`classic`) {
    for (const equipment of equipments) {
        ...
        const displayedItem = (equipment && equipment.transmog && Object.keys(equipment.transmog).length !== 0) ? equipment.transmog : equipment?.item;
