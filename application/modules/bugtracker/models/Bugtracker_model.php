...
class Bugtracker_model extends CI_Model
{
    ...
    public function getBugtracker(): CI_DB_result
    {
        return $this->db->select('*')->where('close', '0')->get('bugtracker');
    }

    public function changePriority($id, $priority): bool
    {
    ...
    public function closeIssue($id): bool
    {
        return $this->db->set('close', '1')->where('id', $id)->update('bugtracker');
        redirect(base_url('bugtracker/report/') . $id, 'refresh');
    }

    public function changeType($id, $type): bool
    {
    ...
    public function getAllBugs(): int
    {
    ...
    public function insertIssue($title, $description, $type, $priority): bool
    {
    ...
    public function getTypes(): CI_DB_result
    {
    ...
    public function getPriorities(): CI_DB_result
    {
    ...
    public function getPriorityGeneral(): CI_DB_result
    {
        return $this->db->select('*')->get('bugtracker_priority');
    }

    public function getStatusGeneral(): CI_DB_result
    {
        return $this->db->select('*')->get('bugtracker_status');
    }

    public function getTypesGeneral(): CI_DB_result
    {
        return $this->db->select('*')->get('bugtracker_type');
    }
    ...
    public function ReportExist($id): int
    {
